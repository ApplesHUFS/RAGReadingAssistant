...
class FAISSIndex:
    ...
    def search_title(self, query: str, threshold: float = 0.25) -> SearchResult:
        ...
        for title in self.titles:
            ...
            if query_only in title_only or title_only in query_only:
                match_length = len(query_only if query_only in title_only else title_only)
                total_length = max(len(query_only), len(title_only))
                confidence = (match_length / total_length) * 100
        ...
        for title in self.titles:
            ...
            query_decomposed = self._decompose_hangul(query_only)
            jamo_ratio = fuzz.ratio(query_decomposed, title_decomposed)
            
            title_tokens = set(self._tokenize_text(title_only))
            query_tokens = set(self._tokenize_text(query_only))
            
            common_tokens = title_tokens & query_tokens
            token_ratio = len(common_tokens) * 100 / max(len(query_tokens), 1)
            
            edit_ratio = fuzz.ratio(query_only, title_only)
            token_sort_ratio = fuzz.token_sort_ratio(query_only, title_only)
        ...
        all_matches = exact_matches + partial_matches + similarity_matches
        ...
        return self._process_matching_results(unique_matches)
    ...
    def _tokenize_text(self, text: str) -> List[str]:
        ...
        morphs = self.komoran.morphs(normalized)
        nouns = self.komoran.nouns(normalized)
